...
namespace Depressurizer.Model
{
    ...
    public sealed class GameList
    {
        ...
        public static XmlDocument FetchXmlGameList(string customUrl) => XmlParser.Load(Resources.UrlCustomGameListXml, customUrl);
        ...
        /// <returns>Fetched XML page as an XmlDocument</returns>
        public static XmlDocument FetchXmlGameList(long steamId) => XmlParser.Load(Resources.UrlGameListXml, steamId);
        ...
        public static string FetchHtmlGameList(string customUrl) => FetchHtmlFromUrl(string.Format(Resources.UrlCustomGameListHtml, customUrl));
        ...
        /// <param name="accountId">The 64-bit account ID</param>
        /// <returns>Full text of the HTTP response</returns>
        public static string FetchHtmlGameList(long accountId) => FetchHtmlFromUrl(string.Format(Resources.UrlGameListHtml, accountId));
        ...
        private void GetLastPlayedFromVdf(VdfFileNode appsNode, SortedSet<int> ignore, AppTypes includedTypes)
        {
            ...
            if (gameNodeArray != null)
            {
                foreach (KeyValuePair<string, VdfFileNode> gameNodePair in gameNodeArray)
                {
                    ...
                    {
                        if (((ignore != null) && ignore.Contains(gameId)) || !Program.GameDatabase.IncludeItemInGameList(gameId, includedTypes))
                        {
                            Logger.Instance.Debug(GlobalStrings.GameData_SkippedProcessingGame, gameId);
                        }
                        else if ((gameNodePair.Value != null) && (gameNodePair.Value.NodeType == ValueType.Array))
                        {
                            ...
                            if (!Games.ContainsKey(gameId))
                            {
                                ...
                                Logger.Instance.Debug(GlobalStrings.GameData_AddedNewGame, gameId, game.Name);
                            }
                            else
                            {
                                game = Games[gameId];
                            ...
                            if (gameNodePair.Value.ContainsKey("LastPlayed") && (gameNodePair.Value["LastPlayed"].NodeInt != 0))
                            {
                                ...
                                Logger.Instance.Debug(GlobalStrings.GameData_ProcessedGame, gameId, Utility.GetDTFromUTime(game.LastPlayed).Date);
                            }
                        }
                    }
                }
                ...
                foreach (KeyValuePair<string, VdfFileNode> gameNodePair in gameNodeArray)
                {
                    ...
                    if (int.TryParse(gameNodePair.Key, out gameId))
                    {
                        ...
                        else if ((gameNodePair.Value != null) && (gameNodePair.Value.NodeType == ValueType.Array))
                        {
                            ...
                            Logger.Instance.Debug(GlobalStrings.GameData_ProcessedGame, gameId, string.Join(",", game.Categories));
                        }
                    }
                }
            }
        ...
        {
            isNew = false;
            if (((ignore != null) && ignore.Contains(appId)) || !Program.GameDatabase.IncludeItemInGameList(appId, includedTypes))
            {
                Logger.Instance.Debug(GlobalStrings.GameData_SkippedIntegratingGame, appId, appName);
                return null;
            }
            ...
            Logger.Instance.Debug(GlobalStrings.GameData_IntegratedGameIntoGameList, appId, appName, isNew);
            return result;
        }
        ...
        public void ExportSteamConfigFile(string filePath, bool discardMissing)
        {
            ...
            if (discardMissing)
            {
                ...
                if (gameNodeArray != null)
                {
                    foreach (KeyValuePair<string, VdfFileNode> pair in gameNodeArray)
                    {
                        ...
                        {
                            Logger.Instance.Debug(GlobalStrings.GameData_RemovingGameCategoryFromSteamConfig, gameId);
            ...
            foreach (GameInfo game in Games.Values)
            {
                if (game.Id > 0)
                {
                    ...
                    Logger.Instance.Debug(GlobalStrings.GameData_AddingGameToConfigFile, game.Id);
            ...
            Logger.Instance.Debug(GlobalStrings.GameData_CleaningUpSteamConfigTree);
